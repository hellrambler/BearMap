...
public class Router {
    ...
    public static List<Long> shortestPath(AugmentedStreetMapGraph g, double stlon, double stlat,
        ...
        return new AStarSolver<>(g, src, dest, 1000).solution();
    }
    ...
    public static List<NavigationDirection> routeDirections(AugmentedStreetMapGraph g,
        ...
        Map<Long, Node> idNode = g.getIdNode();
        List<NavigationDirection> res = new ArrayList<>();

        Long v1 = route.remove(0);
        Node n1 = idNode.get(v1);
        Double prevBearing = null;

        while (route.size() > 0) {
            Long v2 = route.remove(0);
            Node n2 = idNode.get(v2);
            NavigationDirection curr = new NavigationDirection();
            Double currBearing = NavigationDirection.bearing(n1.lon(),n2.lon(),n1.lat(),n2.lat());
            if (prevBearing == null) {
                curr.direction = NavigationDirection.START;
            } else {
                int dir = NavigationDirection.getDirection(prevBearing, currBearing);
                curr.direction = dir;
            }
            curr.distance = NavigationDirection.distance(n1.lon(),n2.lon(),n1.lat(),n2.lat());
            curr.way = n1.name() == null? NavigationDirection.UNKNOWN_ROAD: n1.name();

            if (res.size() == 0) res.add(curr);
            else {
                NavigationDirection prev = res.get(res.size()-1);
                if (curr.direction == NavigationDirection.STRAIGHT) prev.distance += curr.distance;
                else res.add(curr);
            }
